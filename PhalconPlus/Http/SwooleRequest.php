...
class SwooleRequest implements \Phalcon\Http\RequestInterface, \Phalcon\Di\InjectionAwareInterface{
    ...
    public function setDi(\Phalcon\DiInterface $dependencyInjector)
    {
        ...
        if (!$this->_swooleRequest) {
            throw new Request\Exception('swoole request is empty',2314131);
        }
    }
    ...
    {
        $get = isset($this->_swooleRequest->get) ? $this->_swooleRequest->get : [];
        $post = isset($this->_swooleRequest->post) ? $this->_swooleRequest->post : [];

        return array_merge($get,$post);
    }

    public function get($name = null, $filters = null, $defaultValue = null, $notAllowEmpty = false, $noRecursive = false) {
        //$_request = array_merge($this->_swooleRequest->get,$this->_swooleRequest->post);
    ...
    public function getPost($name = null, $filters = null, $defaultValue = null, $notAllowEmpty = false, $noRecursive = false) {
        return $this->getHelper(isset($this->_swooleRequest->post) ? $this->_swooleRequest->post : [],$name,$filters,$defaultValue,$notAllowEmpty,$noRecursive);
    }
    ...
    public function getPut($name = null, $filters = null, $defaultValue = null, $notAllowEmpty = false, $noRecursive = false) {
        ...
        //这个最后来弄
        return '';
        //return $this->getHelper($this->_swooleRequest->,$name,$filters,$defaultValue,$notAllowEmpty,$noRecursive);
    }
    ...
    public function getQuery($name = null, $filters = null, $defaultValue = null, $notAllowEmpty = false, $noRecursive = false) {
        return $this->getHelper((isset($this->_swooleRequest->get) ? $this->_swooleRequest->get: []),$name,$filters,$defaultValue,$notAllowEmpty,$noRecursive);
    }
        ...
        if(null !== $filters){
            ...
            if(!is_object($filter)){
                ...
                if(!is_object($dependencyInjector)){
                    throw new Request\Exception('A dependency injection object is required to access the "filter" service');
                }
    ...
    public function getServer($name) {
        ...
        return isset($this->_swooleRequest->server[$name]) ? $this->_swooleRequest->server[$name] : null;
    }
    ...
    public final function getHeader($header) {
        ...
        return isset($this->_swooleRequest->header[$header]) ? $this->_swooleRequest->header[$header] : '';
    ...
    public function getRawBody() {
        if (empty($this->_rawBody)) {
            $this->_rawBody = $this->_swooleRequest->rawContent();
        }
    ...
    public function isMethod($methods, $strict = false) {
        ...
        if (is_string($methods)) {
            if ($strict && !$this->isValidHttpMethod($methods)) {
                throw new Request\Exception('Invalid HTTP method:' . $methods);
            }
        ...
        if($strict){
            throw new Request\Exception('Invalid HTTP method:non-string');
        }
        return false;
    ...
    public function hasFiles($onlySuccessful = false) {
        ...
        $files = $this->_swooleRequest->files;
        if(!is_array($files)){
            return 0;
        }
    ...
    public function getUploadedFiles($onlySuccessful = false) {
        ...
        $superFiles = $this->_swooleRequest->files;
                ...
                if (is_array($input['name'])) {
                    ...
                    foreach ($smoothInput as $file) {
                        if($onlySuccessful == false || $file['error'] == UPLOAD_ERR_OK){
                            ...
                            $files[] = new Request\File($input, $file['key']);
                        }
                    }

                }else{
                    if ($onlySuccessful == false || $input['error'] == UPLOAD_ERR_OK) {
                        $files[] = new Request\File($input, $prefix);
                    }
                }
            }
        }
    ...
    public function getHeaders() {
        ...
        $headers = [];
        foreach($this->_swooleRequest->header as $key=>$val){
            // abc-def -> Abc Def
